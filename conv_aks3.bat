...
rem bin3\SongToSamples --customSourceProfileFile bin3\rmac.xml --sampleExportOffset 0 --sampleExportAmplitude 16 %1 %2.samples.s
...
rem rem fix bug as of Arkos 3.5 tools: an empty label prefix creates a label called ":"
rem bin\sed -i -e "s/^:$//" %2.raw.linear.s
rem rem strip out all effects
rem bin\sed -i -e "/Effect/d" %2.raw.linear.s
rem bin\sed -i -e "/effects/d" %2.raw.linear.s
rem 
rem rem Same as above, but for samples. Might not be used eventually (we can convert the dc.ws to dc.ls
rem rem and be rid of all the relative pointer nonsense
rem bin\sed -i -e "s/\(dc.w Sample_Sample..\)/\1 - SampleDisarkPointerRegionStart0/" %2.samples.s
rem rem all $ characters should be converted to *
